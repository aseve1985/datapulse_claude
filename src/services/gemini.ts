...
const MODULE_CONTEXTS: Record<string, string> = {
  ...
  celu_ahora: `Estás analizando datos del módulo de CELU-AHORA. Es un producto nuevo de Libgot: un comercio físico en Moreno donde se venden celulares al contado, financiados, o con una combinación de adelanto (contado) + financiado. Columnas clave: precio_lista (precio de venta al contado sin financiar), metodologia_venta_1/monto_venta_1 (primer método de pago — si es "Contado" puede ser un adelanto parcial o el pago total del celular), metodologia_venta_2/monto_venta_2 (segundo método, presente solo si completó el pago con financiación — es null si pagó todo con el método 1). Cuando hubo financiación, además aparecen loan_id, term (plazo), capital, interes e interes con IVA. El foco es: mezcla de métodos de pago (contado vs. financiado vs. combinado), volumen financiado, ticket promedio y evolución de ventas del producto.`,
  ...
  'tesoreria': `Estás analizando datos del módulo de TESORERÍA de Argentina. El dataset contiene 12 filas (una por mes de 2026) con los totales mensuales de la posición de caja. Columnas clave: mes (nombre del mes), estado (cerrado/en_curso/proyectado), saldo_inicio (saldo al inicio del mes en ARS), cobranzas (ingresos operativos totales), cobranzas_presupuesto (cobranzas presupuestadas), originaciones (desembolsos de créditos, valor positivo), proveedores (pagos a proveedores, valor positivo), impuestos (pagos de impuestos, valor positivo), total_egresos (suma de todos los egresos operativos), fco (Flujo de Caja Operativo = cobranzas - total_egresos), saldo_final (saldo proyectado al cierre del mes). Todos los valores monetarios están en pesos argentinos (ARS). Meses con estado "cerrado" son datos reales; "en_curso" es el mes actual (parcialmente real); "proyectado" son estimaciones BE (Best Estimate).`,
